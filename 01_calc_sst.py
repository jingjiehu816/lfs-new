# /data03/hjj/hjj/LFS-new/newwork/region/01_calc_sst.py
...
OBS_BASE_DIR = '/data03/hjj/hjj/DATA/OB_SST_0.1'
...
def process_single_case(case_dir):
    ...
    try:
        c_date_str = re.search(r'case_(\d{8})', os.path.basename(case_dir)).group(1)
        c_date_obj = datetime.strptime(c_date_str, '%Y%m%d')
    except: return []

    results = []
    for day_idx in range(1, 31):
        valid_date = c_date_obj + timedelta(days=day_idx-1)
        if valid_date.month == 2 and valid_date.day == 29: continue 
        
        mod_f = os.path.join(case_dir, f'tt-{valid_date.strftime("%Y-%m-%d")}_10km.nc')
        obs_f = os.path.join(OBS_BASE_DIR, valid_date.strftime("%y%m"), f'{valid_date.strftime("%Y%m%d")}_remap.nc')

        if not (os.path.exists(mod_f) and os.path.exists(obs_f)): continue

        try:
            with xr.open_dataset(mod_f, decode_times=False) as ds_m: mod_d = ds_m['tt'][0, 0].values
            with xr.open_dataset(obs_f, decode_times=False) as ds_o: obs_d = ds_o['analysed_sst'][0].values - 273.15
            
            diff = mod_d - obs_d
            rec = {'case_date': c_date_str, 'forecast_day': day_idx}
    ...
    with mp.Pool(CPU_NUM, initializer=init_worker) as pool:
        for i, res in enumerate(pool.imap_unordered(process_single_case, target_cases)):
            ...
            if (i + 1) % max(int(total / 10), 1) == 0 or (i + 1) == total:
                print(f"    -> SST 进度: {i+1}/{total} ({(i+1)/total*100:.1f}%)", flush=True)

    if not final_res: return

    df = pd.DataFrame(final_res).sort_values(by=['case_date', 'forecast_day'])
    out_csv = os.path.join(DATA_OUT_DIR, f'SST_Evaluation_{START_DATE}-{END_DATE}.csv')
